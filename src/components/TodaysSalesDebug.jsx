    ...
    <div style={{ 
      ...
      {summaryResult && (
        <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#e7f3ff', borderRadius: '4px' }}>
          ...
          <div style={{ fontFamily: 'monospace' }}>
            <div><strong>Total Revenue:</strong> ${summaryResult.totalRevenue?.toFixed(2) || '0.00'}</div>
            <div><strong>Total Transactions:</strong> {summaryResult.totalTransactions || 0}</div>
            <div><strong>Average Transaction:</strong> ${summaryResult.averageTransaction?.toFixed(2) || '0.00'}</div>
            <div><strong>Total Items Sold:</strong> {summaryResult.totalItemsSold || 0}</div>
            <div><strong>Period:</strong> {summaryResult.period}</div>
            {summaryResult.dateRange && (
              <div>
                <strong>Date Range:</strong><br/>
                Start: {summaryResult.dateRange.startLocal}<br/>
        ...
        <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f0f8ff', borderRadius: '4px' }}>
          ...
          {debugResult.error ? (
            ...
            <div style={{ fontFamily: 'monospace', fontSize: '12px' }}>
              <div><strong>Raw Sales Count:</strong> {debugResult.sales?.length || 0}</div>
              <div><strong>Raw Total:</strong> ${debugResult.total?.toFixed(2) || '0.00'}</div>
      ...
      <div style={{ marginTop: '20px', padding: '10px', backgroundColor: '#fff3cd', borderRadius: '4px' }}>
        ...
        <ol style={{ margin: 0, paddingLeft: '20px' }}>
          <li>First run the SQL script: <code>fix_today_sales_data.sql</code></li>
