...
const menu = [
  ...
  { to: "/point-of-sales", label: "Point of Sale", icon: ShoppingCart, color: "orange" },
  ...
  { to: "/reports", label: "Reports", icon: BarChart3, color: "indigo" },
...
export default function Sidebar({ branding }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={`h-16 flex items-center border-b border-gray-200/80 relative ${expanded ? 'justify-between px-6' : 'justify-center'}`}>
        {expanded ? (
          <>
            ...
            return (
              <li key={item.to} className="group relative">
                ...
                >
                  {({ isActive }) => (
                    <>
                      <div className={`flex items-center justify-center transition-transform duration-200 ${isActive ? 'scale-110' : 'group-hover:scale-105'} ${expanded ? '' : 'w-6 h-6'}`}>
      ...
      {/* Enhanced Footer */}
      <div className={`border-t border-gray-200/80 ${expanded ? 'p-5' : 'p-3'}`}>
        {expanded ? (
