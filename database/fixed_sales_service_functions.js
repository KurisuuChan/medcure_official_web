...
export async function getSalesSummary(
  ...
  try {
    ...
    console.log(
      `🕐 Start Date: ${startDate.toISOString()} (Local: ${startDate.toLocaleString()})`
    );
    console.log(
      `🕙 End Date: ${endDate.toISOString()} (Local: ${endDate.toLocaleString()})`
    );
    console.log(
...
export async function debugTodaySales() {
  ...
  const approaches = [
    ...
    },
    {
      ...
      endDate: new Date(
        new Date(now.toDateString()).getTime() + 24 * 60 * 60 * 1000 - 1
      ), // End of today
    },
  ];
  ...
  for (const approach of approaches) {
    ...
    try {
      ...
      console.log(
        `Results: ${sales.length} transactions, $${total.toFixed(2)} revenue`
      );
